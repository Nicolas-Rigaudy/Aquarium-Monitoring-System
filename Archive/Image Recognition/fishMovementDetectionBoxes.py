...
while cap.isOpened() and cv2.waitKey(40) != 27:                                                     #while the video capture is running and the operator has not pressed esc
    ...
    for contour in contours:                                                                        #iterate through each contour
        ...
        if cv2.contourArea(contour) < 500:                                                          #if area is too small, do not show contour box, helps eliminate false positives
            continue
...
averageFishNb = sum(fishNbList)/len(fishNbList)
print (averageFishNb)